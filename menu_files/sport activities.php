<!DOCTYPE html>
<html lang="en">
  <head>
    <title>sports activities</title>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
     <link rel="stylesheet" href="../bootstrap/css/bootstrap.min.css" />
    <link rel="stylesheet" href="../animate/animate.css-main/animate.min.css" />
    <link rel="stylesheet" href="../css/style.css" />
    <link rel="icon" type="image/x-icon" href="../img/logo-g.png" />
  </head>
  <body class="container-fluid">
    <!--php header file included-->
        <?php
        include'../includedphp/header.php';
         ?>
    <!--End php header file included-->
      
    <div class="container">
      <strong><center><p class="mt-2">Sport Activities</p></center></strong>
      <div class="row justify-content-between">
        <div class="col-lg-5">
          <p class=" col-sm-12 mt-2 shadow-lg rounded-2 p-3">Sports play a crucial role in schools and colleges. Participation in various sports activities enables the students to develop their competence level. They also learn the sense of co-operation and co-ordination and also prepare themselves for increasing competition in future life. Likewise various sports activities were conducted in the academic year 2021-2019. The college encourages and extend all sorts of help to students to participate in collage ICT and IUT sports.</p>
        </div>
        <div class="col-lg-7">
          <p class=" col-sm-12 mt-2 shadow-lg rounded-2 p-3">
            Every effort has been taken to strengthen sports the activities of students. Sports, recreation and cultural life,for promoting the organizational and leadership skills of students and simultaneously imbibing them with a sense responsibility towards the society. College sponsors various association managed by the students of the institution. Every effort has been taken to strengthen sports activity inside the campus and making the students participate in inter collegiate and inter university sports meets.
          </p>
        </div>
      </div>
      <div class="row">
        <div class="col-lg-6 col-sm-12">
          <img src="../img/sports_2019.jpg" class="img-fluid m-2 rounded-2 shadow-lg" alt="sports_2019">
        </div>
        <div class="col-lg-5 col-sm-12">
          <img src="../img/sports_20.jpg"   class="img-fluid m-2 rounded-2 shadow-lg" alt="sports_20">
        </div>
      </div>
      </div>
        <div class="row">
        <div class="col-lg-6 col-sm-12">
          <img src="../img/sports_201.jpg" class="img-fluid m-2 rounded-2 shadow-lg" alt="sports_201">
        </div>
        <div class="col-lg-5 col-sm-12">
          <img src="../img/sports_202.jpg" class="img-fluid m-2 rounded-2 shadow-lg" alt="sports_202">
        </div>
        </div>
        <div class="row">

        <div class="col-lg-5 col-sm-12">
          <img src="../img/sports_203.jpg" class="img-fluid m-2 rounded-2 shadow-lg" alt="sports_203">
        </div>
        <div class="col-lg-6 col-sm-12">
          <img src="../img/sports_204.jpg" class="img-fluid m-2 rounded-2 shadow-lg" alt="sports_204">
        </div>
      </div>
      <!-- Sports Achievements 2019-20-->
      <div class="res_table justify-content-center">
        <center>
          <table class="mt-3  table-responsive-lg table-responsive-md table-responsive-sm table-bordered border-warning text-center">
          <thead>
            <tr>
              <td>Sr. No.</td>
              <td>Name of Student</td>
              <td>Class</td>
              <td>Achievement</td>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>1</td>
              <td>Mr.Thakur Akshay</td>
              <td>FYBCA</td>
              <td>Gold medal in 4 × 400m Relay Running in 'C' Zone ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>2</td>
              <td>Mr.Swami Mahesh</td>
              <td>FY B.Sc.(BT)</td>
              <td>Gold medal in 4 × 400m Relay Running in 'C' Zone ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>3</td>
              <td>Mr.Khandre Aditya</td>
              <td>FYBCA</td>
              <td>Gold medal in 4 × 100m Relay Running in 'C' Zone ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>4</td>
              <td>Mr.Jadhav Deepak</td>
              <td>FYBCA</td>
              <td>Gold medal in 4 × 100m Relay Running in 'C' Zone ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>5</td>
              <td>Mr.Lokde Shrinivas</td>
              <td>SY M.Sc.(BI)</td>
              <td>II prize in Inter Zone Cricket Tournament of SRTMUN</td>
            </tr>
            <tr>
              <td>6</td>
              <td>Mr.Sarabjeetsingh Sabbarwa</td>
              <td>FYBCA</td>
              <td>II prize in Inter Zone Cricket Tournament of SRTMUN</td>
            </tr>
            <tr>
              <td>7</td>
              <td>Ms.Gundale Ankita</td>
              <td>FTYBCA</td>
              <td>III Prize in Basketball(Women) in ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>8</td>
              <td>Ms.Khandre Meghna</td>
              <td>TYBCA</td>
              <td>III Prize in Basketball(Women) in ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>9</td>
              <td>Ms.Kamble Mohini</td>
              <td>TYBCA</td>
              <td>III Prize in Basketball(Women) in ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>10</td>
              <td>	Ms.Dhangare Prerana</td>
              <td>FYBCA</td>
              <td>III Prize in Basketball(Women) in ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>11</td>
              <td>Ms.Yadav Mamta</td>
              <td>TY.B.Sc.(CS)</td>
              <td>III Prize in Basketball(Women) in ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>12</td>
              <td>	Ms.Indurkar Priyanka</td>
              <td>TY.B.Sc.(CS)</td>
              <td>III Prize in Basketball(Women) in ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>13</td>
              <td>	Ms.Nisang Sonal</td>
              <td>TY.B.Sc.(CS)</td>
              <td>III Prize in Basketball(Women) in ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>14</td>
              <td>Ms.Yelakadre Mayuri</td>
              <td>FYBCA</td>
              <td>III Prize in Basketball(Women) in ICT of SRTMUN</td>
            </tr>
            <tr>
              <td>15</td>
              <td>Mr.Akash L.Talmute</td>
              <td>FYBCA</td>
              <td>Bronze Medal in Shotput in MGM's Olympics 2019, Aurangabad</td>
            </tr>
            <tr>
              <td>16</td>
              <td>Mr.Atharv Sirsalkar</td>
              <td>FY.B.Sc.(CS)</td>
              <td>Silver medal in 50m freestyle & 50m backstroke swimming event in MGM's Olympics 2019,Aurangabad</td>
            </tr>
            <tr>
              <td>17</td>
              <td>	Mr.Santosh G.Minmulwar</td>
              <td>TYBCA</td>
              <td>II prize in Volleyball in MGM's Olympics 2019 ,Aurangabad</td>
            </tr>
            <tr>
              <td>18</td>
              <td>Mr.Datta G.Giri</td>
              <td>SY.B.Sc.(CS)</td>
              <td>II prize in Volleyball in MGM's Olympics 2019 ,Aurangabad</td>
            </tr>
            <tr>
              <td>19</td>
              <td>Mr.Krishna Jadhav</td>
              <td>SY.B.Sc.(CS)</td>
              <td>II prize in Volleyball in MGM's Olympics 2019 ,Aurangabad</td>
            </tr>
            <tr>
              <td>20</td>
              <td>Mr.Suraj Reddy Lokawad</td>
              <td>FYBCA</td>
              <td>II prize in Volleyball in MGM's Olympics 2019 ,Aurangabad</td>
            </tr>
            <tr>
              <td>21</td>
              <td>	Mr.Nagesh Kamble</td>
              <td>FYBCA</td>
              <td>II prize in Volleyball in MGM's Olympics 2019 ,Aurangabad</td>
            </tr>
            <tr>
              <td>22</td>
              <td>	Mr.Sanket Solanke</td>
              <td>TYBCA</td>
              <td>II prize in Volleyball in MGM's Olympics 2019 ,Aurangabad</td>
            </tr>
            <tr>
              <td>23</td>
              <td>Mr.Sagar Pawar</td>
              <td>SY M.Sc.(BT)</td>
              <td>II prize in Volleyball in MGM's Olympics 2019 ,Aurangabad</td>
            </tr>
            <tr>
              <td>24</td>
              <td>	Mr.Ramdas Narwade</td>
              <td>SY M.Sc.(BT)</td>
              <td>II prize in Volleyball in MGM's Olympics 2019 ,Aurangabad</td>
            </tr>
          </tbody>
        </table>
        </center>
      </div>
    


     <!--php footer-->
        <?php
        include'../includedphp/footer.php';
        ?>
    <!--End php footer-->
  </body>
<script src="../bootstrap/js/bootstrap.bundle.min.js"></script>
</html>